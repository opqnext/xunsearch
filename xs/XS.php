...
class XS extends XSComponent
{
    ...
    public function __construct($file)
    {
        if (strlen($file) < 255 && !is_file($file)) {
            ...
            if ($appRoot === false) {
                $appRoot = defined('XS_APP_ROOT') ? XS_APP_ROOT : XS_LIB_ROOT . '';
            }
    ...
    public static function convert($data, $to, $from)
    {
        ...
        if (is_string($data) && preg_match('/[\x81-\xfe]/', $data)) {
            if (function_exists('mb_convert_encoding')) {
                ...
                throw new \Exception('Cann\'t find the mbstring or iconv extension to convert encoding');
            }
        }
        return $data;
    }
    ...
    private function loadIniFile($file)
    {
        ...
        if ($this->_config === false) {
            throw new \Exception('Failed to parse project config file/string: \'' . substr($file, 0, 10) . '...\'');
        }
    ...
    public function addField($field, $config = null)
    {
        ...
        if ($field->isSpeical()) {
            if (isset($this->_typeMap[$field->type])) {
                ...
                throw new \Exception('Duplicated ' . strtoupper($config['type']) . ' field: `' . $field->name . '\' and `' . $prev . '\'');
            }
    ...
    public function getCustomTokenizer()
    {
        if (isset(self::$_tokenizers[$this->tokenizer])) {
            ...
            if (($pos1 = strpos($this->tokenizer, '(')) !== false
                ...
                $name = 'core\search\XSTokenizer' . ucfirst(trim(substr($this->tokenizer, 0, $pos1)));
                $arg = substr($this->tokenizer, $pos1 + 1, $pos2 - $pos1 - 1);
            } else {
                $name = 'core\search\XSTokenizer' . ucfirst($this->tokenizer);
                $arg = null;
            }
            if (!class_exists($name)) {
                ...
                if (!class_exists($name)) {
                    throw new \Exception('Undefined custom tokenizer `' . $this->tokenizer . '\' for field `' . $this->name . '\'');
                }
            }
            $obj = $arg === null ? new $name : new $name($arg);
            if (!$obj instanceof XSTokenizer) {
                throw new \Exception($name . ' for field `' . $this->name . '\' dose not implement the interface: XSTokenizer');
            }
    ...
    public function setFacets($field, $exact = false)
    {
        ...
        foreach ($field as $name) {
            ...
            if ($ff->type !== XSFieldMeta::TYPE_STRING) {
                throw new \Exception("Field `$name' cann't be used for facets search, can only be string type");
            }
            $buf .= chr($ff->vno);
        }
    ...
    public function __construct($arg = null)
    {
        if ($arg !== null && $arg !== '') {
            ...
            if ($this->arg < 1 || $this->arg > 255) {
                throw new \Exception('Invalid argument for ' . __CLASS__ . ': ' . $arg);
            }
        }
    }
